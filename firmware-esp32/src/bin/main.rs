...
// (ssid, password) pairs from secrets.env, generated by build.rs. Tried in
// order on each connect attempt; see `wifi_task`.
...
async fn wifi_task(mut controller: WifiController<'static>) {
    // Index of the network to try next. Kept across disconnects so a drop
    // retries the network that was working before moving on to the others.
    let mut idx = 0usize;

    loop {
        if WIFI_NETWORKS.is_empty() {
            info!("No WiFi networks configured; WiFi task idle");
            return;
        }

        let (ssid, password) = WIFI_NETWORKS[idx];
        info!(
        ...
        if let Err(e) = controller.set_config(&ModeConfig::Client(
            ClientConfig::default()
                .with_ssid(ssid.to_string())
                .with_password(password.to_string()),
        )) {
            info!("WiFi config for {ssid} failed: {e:?}");
            idx = (idx + 1) % WIFI_NETWORKS.len();
            continue;
        }
        ...
        match controller.connect_async().await {
            Ok(()) => {
                ...
                controller
                    ...
                    .await;
                info!("WiFi disconnected from {ssid}, reconnecting...");
            }
            Err(e) => {
                info!("WiFi connect to {ssid} failed: {e:?}");
                // Move on to the next network. Only pause once a full pass has
                // failed, so scanning through several networks stays quick but
                // an all-down network doesn't spin.
                idx = (idx + 1) % WIFI_NETWORKS.len();
                if idx == 0 {
                    info!("All WiFi networks failed, retrying in 5s");
...
async fn main(spawner: Spawner) -> ! {
    ...
    // The SSID/password are set per-attempt inside `wifi_task`, which cycles
    // through WIFI_NETWORKS; the controller only needs starting here. `start()`
    // requires a config, so seed it with the first network.
    if let Some((ssid, password)) = WIFI_NETWORKS.first() {
        wifi_controller
